// Add the transparency page to the shared navigation on existing pages.
const siteNav = document.querySelector('header nav');
if (siteNav && !siteNav.querySelector('a[href="transparency.html"]')) {
    const transparencyLink = document.createElement('a');
    transparencyLink.href = 'transparency.html';
    transparencyLink.textContent = 'Transparency';
    siteNav.insertBefore(transparencyLink, siteNav.querySelector('a[href="#contact"]'));
}
...
if (document.getElementById('contactForm')) {
    document.getElementById('contactForm').addEventListener('submit', function(e) {
        ...
        window.location.href = `mailto:info@vegas-all.org?subject=${subject}&body=${body}`;
...
if (chatToggle && chatWidget && chatClose && chatSend && chatInput && chatMessages) {
    ...
    const botResponses = {
        'hello': 'Hi there! How can we help you today?', 'hi': 'Hello! Welcome to Vegas ALL. What can I do for you?',
    ...
    function sendMessage() {
        ...
        chatInput.value = '';
        setTimeout(() => {
            const key = Object.keys(botResponses).find(item => userMessage.toLowerCase().includes(item)) || 'default';
            const botMsgDiv = document.createElement('div');
            botMsgDiv.className = 'message bot';
            botMsgDiv.innerHTML = `<div class="message-content">${botResponses[key]}</div>`;
